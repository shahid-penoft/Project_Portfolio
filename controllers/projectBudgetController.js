    ...
    try {
        ...
        const [result] = await db.query(
            `INSERT INTO project_budget_entries (project_id, csr_org_id, category, amount, period)
             VALUES (?, ?, ?, ?, ?)`,
            [id, csr_org_id || null, category.trim(), parseFloat(amount) || 0, period || null]
        );
        ...
        if (csr_org_id) {
            await db.query(
                `UPDATE csr_project_links SET spent_amount = spent_amount + ? WHERE csr_org_id = ? AND project_id = ?`,
                [parseFloat(amount) || 0, csr_org_id, id]
            );
        }
    ...
    try {
        ...
        const [result] = await db.query(
            `INSERT INTO project_budget_allocations (project_id, fund_source, csr_org_id, category, amount, period)
             VALUES (?, ?, ?, ?, ?, ?)`,
            [id, fund_source?.trim() || null, csr_org_id || null, category.trim(), parseFloat(amount) || 0, period?.trim() || null]
        );
